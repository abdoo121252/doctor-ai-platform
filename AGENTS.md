...
pnpm test:m365     # comprehensive Microsoft 365 tool test (mail, calendar, drive, contacts, todo, search)
```
...
| Outlook contacts calls | `packages/agent/src/microsoft/contacts.ts` |
| Microsoft To Do calls | `packages/agent/src/microsoft/todo.ts` |
| Unified Microsoft Search | `packages/agent/src/microsoft/search.ts` (`searchM365` — Graph `/search/query`, Azure AD accounts only) |
| Microsoft tool factories | `packages/agent/src/microsoft/tools/` (mail.ts, calendar.ts, drive.ts, contacts.ts, todo.ts, search.ts) |
| Microsoft 365 tool tests | `scripts/test-m365-agent.ts` (`pnpm test:m365`) |
...
| Multi-path routing (AI selects path) | `packages/agent/src/filter.ts` (`routeEventToPath`) |
| Multi-path event trigger migration | `supabase/migrations/00013_event_trigger_paths.sql` (`event_triggers.paths` jsonb, backfilled into a single "Default" path) |
...
## Phase conventions
- **Phase 5+:** All tools call real Google/Microsoft APIs via `googleapis` / Microsoft Graph. Use `getGoogleAuth(doctorId, supabaseClient?)` → fetches encrypted refresh token from `google_connections`, creates OAuth2 client, auto-refreshes tokens. Pass the caller's Supabase client when available so it works without `SUPABASE_SERVICE_KEY`.
- **Google tool surface:** `packages/agent/src/google/` covers Gmail (read/send/search/labels/filters/drafts), Calendar (events/calendars/freebusy/OOO/focus time), Drive (search/content/download/create/share/permissions), Sheets (read/write/format/batch), Docs (search/get/export/comments/images/paragraphs/headers-footers/tables), Slides (get/batch/thumbnails/comments), Forms (get/batch/responses/publish), Tasks, Contacts (list/get/search/manage/groups), and Google Chat/Spaces (list/messages/send/search/reactions).
- **Env vars:** `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `GOOGLE_ENCRYPTION_KEY` (must be exactly 32 chars).
...
- **Microsoft 365 integration (expanded — 2026-08-21):** migration `00004_microsoft_connections.sql` (table + RLS + partial unique index), `packages/agent/src/microsoft/` (auth.ts, graph.ts, mail.ts, calendar.ts, drive.ts, contacts.ts, todo.ts, search.ts) + tool factories in `microsoft/tools/`. The agent exposes ~60 Microsoft tools across: Outlook mail (read/send/reply/drafts/folders/rules/categories/focused inbox), Outlook calendar (read/create/update/delete + recurrence + online meetings), OneDrive (search/list/get/read/download/upload/delete/share/move/copy/create folder), Outlook contacts (list/search/get/create/update/delete + folders), Microsoft To Do (lists/tasks/checklist items), and unified Microsoft Search `searchM365` (Graph `/search/query` across message/event/driveItem/listItem/person; work/school Azure AD accounts only — MSA/personal accounts get a clear error). New routes: `GET /api/settings/microsoft-connect`, `GET /api/settings/microsoft-connection`, `GET /api/auth/microsoft-callback` (public). Settings page has a Microsoft card + Outlook/OneDrive service rows. Env: `MICROSOFT_CLIENT_ID`, `MICROSOFT_CLIENT_SECRET`, `MICROSOFT_REDIRECT_URI` (placeholders added to `.env.local` — must be filled from Azure portal App Registration). Same `GOOGLE_ENCRYPTION_KEY` encrypts both providers' tokens. Agent + web typechecks pass. TO CONNECT: register an Azure app (redirect URI `http://localhost:3000/api/auth/microsoft-callback`, scopes Mail.ReadWrite, Mail.Send, Calendars.ReadWrite, Files.ReadWrite, User.Read, offline_access), fill env, restart web, click Connect in Settings.
...
- **Multi-path event triggers — smart AI routing (NEW — 2026-08-21):** the event trigger form now builds **paths** instead of the single `instructions` + field filter + `condition`. Each path = `{ id, name?, filter, instructions }` where `filter` is either `{ mode: "fields", fields: EventFilterRules }` (deterministic) or `{ mode: "ai", condition }` (natural language). OR between paths, AND within a path's fields. Stored in `event_triggers.paths` (jsonb, migration `00013_event_trigger_paths.sql`, existing rows backfilled into one `"Default"` path: AI mode if a `condition` existed, else fields mode). **Routing** happens in `runAutomationPayload` (`apps/web/src/lib/automation-dispatch.ts`): walk paths in order — fields-mode matched cheaply via `doesEventMatchFilter` (no AI call, first match wins); if none matched and AI paths exist, ONE `routeEventToPath` call (`packages/agent/src/filter.ts`) evaluates ALL AI conditions and returns the selected `pathId` (NOT yes/no); the selected path's `instructions` drive the agent; no match → skip (`no_path_matched`). Legacy triggers with empty `paths` keep the old `condition`→`filterMatchesCondition` yes/no flow. `POST/PATCH /api/events` accept `paths`; the poll + worker forward `paths` to `/api/automation/run`; `createEventTriggerTool` accepts either legacy fields or `paths`. The `/tasks` event form has a per-path filter-mode toggle (Fields / AI) with source-aware field inputs or a natural-language condition textarea + instructions.
